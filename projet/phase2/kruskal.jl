...
Fonction réalisant l'union de deux arborescences, par l'arête edge,
 ...
 """
function union!(edge::Edge{T},dict::Dict{String,String}) where T
    #On récupère dans node1 et node2 les deux noeuds contenus dans l'arête edge
    ...
    p1=node2
    p2=dict[node2]
    #On remplace le parent de node2 par node1, c'est-à-dire qu'on supprime l'arc allant de p1 vers p2 et on le remplace vers un arc allant de p1 vers node1
    dict[node2]=node1
    #Tant que le parent de p1 est différent de lui même (c'est-à-dire tant que l'on n'a pas atteint la racine de la deuxième arborescence)
    while p1!=p2
        #avant d'appliquer des changements au dictionaire, on mémorise le parent de p2
        p3=dict[p2]
        #on peut alors appliquer les changements au dictionaire: l'arc reliant p1 et p2 change de sens, et l'arc reliant p2 et son ancient parent p3 disparait
        dict[p2]=p1
...
function kruskal(G::Graph)
     ...
     for e in E
         if data(e)[1] in nodes(G)==false
             println("erreur")
         end
         if data(e)[2] in nodes(G)==false
             println("erreur")
         end
     end
